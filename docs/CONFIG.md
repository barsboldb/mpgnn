...
- `gnn` — message passing (`gcn`/`sage`/`gat`/`gin`). Built by `src.model.GNN`.
...
- `node` (default) — vertices are the only tokens. With `model: gnn` edges enter via message passing (`gcn`/`gin`/`gat`/`sage`); with `model: transformer` they enter via all-pairs `global_attn` (plus the SPD bias and/or LPE). Built by `src.model.GNN` (the shared conv engine, which `GraphTransformer` reuses for `node` tokenization).
