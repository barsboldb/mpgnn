...
This is the Sanford-style tokenization in `token_model.py`. We build one token per vertex, one token per edge, and a single "task" token that reads out the answer:
